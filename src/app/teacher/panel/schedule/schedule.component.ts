...
export class ScheduleComponent {
  ...
  eventContent(eventInfo: any) {
    return {
      html: `
        <div style="font-weight:bold;">${eventInfo.event.title.split('|')[0].trim()}</div>
        <div style="font-size: 0.8em;">${eventInfo.event.title.split('|')[1].trim()}</div>
      `
    };
  }
      ...
      .subscribe(
        data => {
          ...
          const events: EventInput[] = data.map(item => ({
            title: `${item.courseName} | ${item.gradeName} ${item.levelName} 
            ${item.sectionName} - ${item.classroomName}`,
            
            daysOfWeek: [dowMap[item.day] || 1],
            startTime:  item.startTime.slice(0,5),  // "HH:mm"
            endTime:    item.endTime.slice(0,5),
            color:      this.courseColors[item.courseName] ?? this.courseColors['_default']
